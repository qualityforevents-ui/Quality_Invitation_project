...
export function ModernCover({
  ...
  return (
    <motion.div
      className="relative flex min-h-dvh flex-col items-center justify-center bg-inv-bg px-10 py-16 text-center"
      exit={{ opacity: 0, y: -18 }}
    ...
    >
      ...
      <motion.h1
        className="flex flex-col items-center gap-3 font-inv-display font-light text-inv-ink"
        initial={{ opacity: 0, letterSpacing: '0.3em' }}
        animate={{ opacity: 1, letterSpacing: '0.02em' }}
        transition={{ duration: 1.4, delay: 0.25, ease: EASE }}
      >
        <span className="block text-[2.25rem] leading-tight text-balance">{view.name1}</span>
        <span className="block text-[2.25rem] leading-tight text-balance">{view.name2}</span>
      </motion.h1>
      ...
      >
        ...
        <p className="font-inv-body text-sm tracking-[0.22em] text-inv-ink">
      ...
      <motion.button
        type="button"
        onClick={onOpen}
        className="tap-target mt-14 border-b border-inv-ink pb-1.5 font-inv-body text-sm tracking-[0.25em] text-inv-ink uppercase transition active:opacity-60"
