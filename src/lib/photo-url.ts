/**
 * The half of the ImageKit integration that is safe in a browser bundle.
 *
 * Split out of `src/lib/imagekit.ts`, whose first line is `import { createHmac } from
 * 'node:crypto'` for signing uploads. That was harmless while the only client code
 * touching it imported `type { PhotoCrop }`, which the compiler erases. The one page
 * flow builds an invitation view on the client so it can hand the preview a photo
 * without a round trip, and the moment anything imports a real value from that module
 * the Postgres style failure follows: node:crypto is dragged into the browser bundle
 * and the build dies on a module the browser has never heard of.
 *
 * So the rule is a file boundary rather than a comment: nothing here reads a private
 * key or touches node builtins, and client code imports from here. `imagekit.ts`
 * re-exports these for the server callers that already point at it.
 */
